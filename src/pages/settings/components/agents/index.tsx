    ...
    const handleEditAgent = (agent: SettingA2AServer) => {
        ...
        form.setFieldsValue({
            ...
            custom_header_json: agent.customHeaderJson ? toPrettyJsonString(JSON.parse(agent.customHeaderJson)) : toPrettyJsonString({
                "X-A2A-Client-UI-Version": "v0.1.0"
            })
        });
        onShowAgentModal();
    };
                        ...
                        help={
                            <div>
                                <div style={{ marginBottom: '2px' }}>
                                    Configure custom HTTP headers that will be sent with every request to this A2A server.
                                </div>
                                <div style={{ fontSize: '11px', color: '#9ca3af' }}>
                                    <strong>Common use cases:</strong>
                                    <br />• Authorization: Bearer tokens, API keys
                                    <br />• Custom headers: X-A2A-Client-UI-Version, X-Client-ID
                                    <br />• Content negotiation: Accept, Content-Type
                                </div>
                            </div>
                        }
                    >
